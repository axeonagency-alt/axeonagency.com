...
function initMobile() {
  ...
  /* Prevent multiple execution if script is included twice */
  if (window.__axeonMobileJsLoaded) return;
  window.__axeonMobileJsLoaded = true;
  ...
  if (!navbar) {
    ...
    navbar.className = 'navbar fade-up';
    navbar.innerHTML = `
      <a href="/" class="nav-logo">axeon.</a>
      <div class="nav-links">
        <a href="/about" class="nav-link">About</a>
        <a href="/faqs" class="nav-link">FAQs</a>
        <a href="/work" class="nav-link">Work</a>
        <a href="/blog" class="nav-link">Blog</a>
        <a href="/contact" class="nav-link btn btn-pink" style="padding: 0.6rem 1.6rem;">Contact Us</a>
      </div>
    `;
    document.body.prepend(navbar);
  }

  let navLinksContainer = navbar.querySelector('.nav-links');
  ...
  if (navbar && navLinksContainer) {
    ...
    if (navLogo && !navLogo.querySelector('.nav-logo-text-container')) {
      ...
      html += `<img src="logo.png" alt="Axeon Logo" class="nav-logo-img" style="position: absolute; left: 0; top: 50%; transform: translateY(-50%); height: 6.5rem; width: auto; opacity: 0; transition: opacity 0.4s ease 0.3s; pointer-events: none; object-fit: contain;">`;
    ...
    let hamburger = navbar.querySelector('.mobile-menu-btn');
    if (!hamburger) {
