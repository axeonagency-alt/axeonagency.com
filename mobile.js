 ...
 *
 * Key mobile-performance principles applied here (inspired by
 * top-tier Framer / production sites like Budarina Studios):
 *   1. Lenis is DISABLED on touch devices — native scroll is faster.
 *   2. GSAP ScrollTrigger scrub animations are simplified on mobile.
 *   3. All scroll listeners are { passive: true }.
 *   4. Navbar hide/show uses requestAnimationFrame debouncing.
 *   5. touch-action: manipulation is set in CSS, but double-tap
 *      prevention is also handled in JS for older browsers.
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ── 2. DISABLE LENIS ON TOUCH DEVICES ──────────────────── */
  /*
   * Lenis smooth-scroll uses requestAnimationFrame to lerp the
   * scroll position, which fights the browser's native compositor
   * thread on mobile and causes visible stutter / jank.
   * On touch devices we destroy Lenis and let the browser do its job.
   *
   * On desktop (mouse), Lenis is left running from the inline script
   * in each HTML file — we don't touch it here.
   */
  if (isTouch() && typeof window.lenis !== 'undefined') {
    try {
      window.lenis.destroy();
      // Remove the lenis RAF loop
      document.documentElement.classList.remove('lenis', 'lenis-smooth');
      document.body.classList.remove('lenis', 'lenis-smooth');
    } catch (e) { /* lenis may not be available on every page */ }
  }

  /*
   * Also kill Lenis if it's still in the global scope under `lenis`
   * (the inline script in each page creates it as a const, which is
   * block-scoped — but the RAF loop runs indefinitely).
   * We patch cancelAnimationFrame indirectly by pausing the instance.
   */
  if (isTouch()) {
    // Try to reach the lenis instance created in-page
    const globalLenis = window.lenis || (typeof lenis !== 'undefined' ? lenis : null);
    if (globalLenis && typeof globalLenis.stop === 'function') {
      try { globalLenis.stop(); } catch(e) {}
    }
    // Restore native scroll behaviour in case Lenis locked it
  ...
  if (navbar && navLinksContainer) {
    ...
    /* Fix link labels if they were stripped by split-text animation */
    if (isMobile()) {
    ...
    /*
     * Uses RAF-debouncing (ticking flag) so the scroll handler
     * never fires more than once per frame — a critical mobile
     * perf pattern. All reads happen before writes (avoid layout thrashing).
     */
    let lastScrollY = 0;
    let ticking     = false;
  ...
  /*
   * `touch-action: manipulation` in CSS is the modern solution.
   * This JS fallback covers older Android browsers.
   */
  let lastTouchEnd = 0;
  ...
  /* ── 12. GSAP MOBILE PERFORMANCE PATCH ─────────────────── */
  /*
   * On mobile, GSAP's scrub-based ScrollTrigger animations (e.g. the
   * img-container parallax with scrub:1) fire on every scroll tick
   * and force layout recalculations. We disable them on touch and
   * replace with simpler one-shot entrance animations.
   */
  if (isTouch() && typeof ScrollTrigger !== 'undefined') {
    // Kill all scrub-based triggers that are continuous (they stutter on mobile)
    ...
    // Immediately set animated properties to their final state
    // so elements don't stay invisible after the trigger is killed
  ...
  /* ── 13. PASSIVE PERFORMANCE: document-level ────────────── */
